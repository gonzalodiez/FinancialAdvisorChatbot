...
class PDFProcessor:
    ...
    def __init__(self, remove_numeric_tables=True, 
        ...
        self.converter = PDFToTextConverter(remove_numeric_tables=remove_numeric_tables,
                                            valid_languages=valid_languages)
        self.preprocessor = PreProcessor(   clean_empty_lines=True,
...
class Dataset:
    ...
    def load_and_process_folder(self):
        ...
        for pdf_file in pdf_files:
            ...
            # Get usefull info from the name
...
metadata_df = pd.json_normalize(processed_df['meta'])
processed_df = pd.concat([processed_df.drop('meta', axis=1), metadata_df], axis=1)


# Save the DataFrame to a CSV file with tab ('\t') as the delimiter and escape special characters
os.makedirs(pdf_folder_path, exist_ok=True)
processed_df.to_csv('prueba/data.csv')